...
opt.hiddenSize = 6
opt.numLayers = 1
opt.useSeqLSTM = false -- faster implementation of LSTM + Sequencer
opt.vocabSize = 7
...
function forwardConnect(enc, dec)
   for i=1,#enc.lstmLayers do
      if opt.useSeqLSTM then
         ...
         dec.lstmLayers[i].userPrevOutput = nn.utils.recursiveCopy(dec.lstmLayers[i].userPrevOutput, enc.lstmLayers[i].outputs[opt.seqLen])
         dec.lstmLayers[i].userPrevCell = nn.utils.recursiveCopy(dec.lstmLayers[i].userPrevCell, enc.lstmLayers[i].cells[opt.seqLen])
      end
   end
end
